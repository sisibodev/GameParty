...
// ─── Single simulation run for a given player character ───────────────────────
function simulateRunForChar(runIndex, playerChar) {
  const PLAYER_ID = playerChar.id;
...
const ARCHETYPES = ['tank', 'berserker', 'assassin', 'ranger', 'mage', 'paladin', 'support', 'warrior'];
const NUM_RUNS   = 1000;

console.log('Running per-archetype simulations (1000 runs each)...\n');
const totalStart = Date.now();

const archetypeResults = {};

for (const archetype of ARCHETYPES) {
  const playerChar = CHARACTERS.find(c => c.archetype === archetype);
  if (!playerChar) {
    console.log(`WARNING: No character found for archetype '${archetype}', skipping.`);
    continue;
  }

  process.stdout.write(`  [${archetype.padEnd(10)}] id=${playerChar.id} — simulating...`);
  const arcStart = Date.now();

  const runs = [];
  for (let i = 0; i < NUM_RUNS; i++) {
    runs.push(simulateRunForChar(i, playerChar));
  }

  const elapsed = ((Date.now() - arcStart) / 1000).toFixed(1);
  process.stdout.write(` done (${elapsed}s)\n`);

  // Aggregate
  const wonRuns     = runs.filter(r => r.firstWinRound !== null);
  const winRounds   = wonRuns.map(r => r.firstWinRound);
  const winRate     = wonRuns.length / NUM_RUNS;

  const avgWinRound = winRounds.length > 0
    ? winRounds.reduce((a, b) => a + b, 0) / winRounds.length
    : null;

  // Stage distribution: last stage reached per run
  const stageDist = { qualifier_out: 0, group_out: 0, tournament_out: 0, finalist: 0, winner: 0 };
  for (const run of runs) {
    const last = run.stagesPerRound[run.stagesPerRound.length - 1];
    if (last && stageDist[last] !== undefined) stageDist[last]++;
  }
...
console.log(`\nAll archetypes done in ${(totalMs / 1000).toFixed(1)}s\n`);

// ─── Report ───────────────────────────────────────────────────────────────────
const sorted = Object.values(archetypeResults).sort((a, b) => b.winRate - a.winRate);

console.log('═══════════════════════════════════════════════════════════════════════════════════');
console.log('         Training Arena — Per-Archetype Simulation Report (1000 runs each)        ');
console.log('═══════════════════════════════════════════════════════════════════════════════════');
console.log(`NPC growthStats: all ${NPC_BASE_GROWTH}+(N-1) per stat at round N  |  Player starts: all ${INITIAL_PLAYER_STAT}`);
console.log('');

console.log('─── Win Rate Ranking ───────────────────────────────────────────────────────────────');
console.log('  Rank  Archetype    CharID  WinRate   Wins/1000  AvgWinRound  Bar');
console.log('  ────  ──────────   ──────  ────────  ─────────  ───────────  ───');
for (let i = 0; i < sorted.length; i++) {
  const r      = sorted[i];
  const rank   = String(i + 1).padStart(4);
  const arch   = r.archetype.padEnd(10);
  const cid    = String(r.charId).padStart(4);
  const pct    = (r.winRate * 100).toFixed(1).padStart(6) + '%';
  const wins   = String(r.wins).padStart(7);
  const avgRnd = r.avgWinRound !== null ? r.avgWinRound.toFixed(2).padStart(9) : '      N/A';
  const bar    = '█'.repeat(Math.round(r.winRate * 50));
  console.log(`  ${rank}  ${arch}   ${cid}  ${pct}  ${wins}    ${avgRnd}    ${bar}`);
}

console.log('');
console.log('─── Stage Distribution (count out of 1000 runs, last stage reached per run) ────────');
console.log('  Archetype    | qual_out | grp_out | tourn_out | finalist | winner');
console.log('  ─────────────┼──────────┼─────────┼───────────┼──────────┼────────');
for (const r of sorted) {
  const arch = r.archetype.padEnd(11);
  const d    = r.stageDist;
  const fmt  = (k) => String(d[k] || 0).padStart(6);
  console.log(`  ${arch}  | ${fmt('qualifier_out')}   | ${fmt('group_out')}  | ${fmt('tournament_out')}      | ${fmt('finalist')}   | ${fmt('winner')}`);
}
...
for (const r of sorted) {
  const arch = r.archetype.padEnd(11);
  const d    = r.stageDist;
  const fmt  = (k) => ((d[k] || 0) / r.numRuns * 100).toFixed(1).padStart(5) + '%';
  console.log(`  ${arch}  | ${fmt('qualifier_out')}  | ${fmt('group_out')} | ${fmt('tournament_out')}    | ${fmt('finalist')} | ${fmt('winner')}`);
}
